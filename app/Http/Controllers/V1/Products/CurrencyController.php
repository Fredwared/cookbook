...
use App\Http\Requests\V1\Products\StoreCurrencyRequest;
...
class CurrencyController extends Controller
{
     ...
     *
     */
    public function store(StoreCurrencyRequest $request, CurrencyService $currencyService): ApiCreatedResponse|JsonResponse
    {
        try {
            $currency = $currencyService->storeCurrency($request->validated());
            $data = CurrencyResource::make($currency);

            return new ApiCreatedResponse($data, 'Currency created successfully');
